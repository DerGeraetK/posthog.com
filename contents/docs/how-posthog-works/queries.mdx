...
By default, insights filter on person properties as they were at event time. You can override this per insight using **Person property mode** in **Advanced options**.

There are three options:

- **Project default** - Uses your project's default setting
- **Event-time snapshot** - Uses person properties as they were when the event was captured
- **Latest person profile** - Joins the current person profile at query time, so filters reflect the person's current properties rather than their properties at event time

When you select **Latest person profile**, a warning appears because changing a person's properties can alter historical-looking results. For example, if you update a user's `plan` property from `free` to `enterprise`, past events for that user now match filters for `enterprise` users, even though they were on the `free` plan when those events occurred.
