...
PostHog mints a dedicated, read-only credential scoped to skills only (`llm_skill:read`) and drops it into ready-to-paste install commands for both Claude Code and Codex. The credential is yours alone, so rotating it never affects a teammate, and you can revoke it anytime from **Settings** > **Project secret API keys**.

The credential is shown only once. It's read-only, reads this project's skills and nothing else, and is never your personal API key.
...
```text
/plugin marketplace add https://x-access-token:YOUR_PHS_TOKEN@app.posthog.com/api/projects/:team_id/llm_skills/marketplace.git
...
```bash
codex plugin marketplace add "https://x-access-token:YOUR_PHS_TOKEN@app.posthog.com/api/projects/:team_id/llm_skills/marketplace.git"
...
You can round-trip any stored skill to a spec-compliant `SKILL.md` directory. From the skills list or a skill's detail page, click **Download .zip** to export the skill — its frontmatter, body, and bundled files — as a zip. To bring a skill in, drop a `SKILL.md` directory zip back into the UI to import it. This is handy for moving a skill in or out of PostHog, or sharing it with an agent that reads `SKILL.md` directories directly.
