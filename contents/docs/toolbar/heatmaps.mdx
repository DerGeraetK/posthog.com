...
## Analyze heatmaps with PostHog AI

[PostHog AI](/docs/posthog-ai) can assess your heatmap data and recommend concrete page changes. From the [Web Analytics](https://app.posthog.com/web) scene, ask PostHog AI to analyze a page and it pulls:

- **Click and rageclick hotspots** – where users click and where they click repeatedly out of frustration
- **Scroll depth** – how far visitors scroll and where they drop off
- **Element identification** – cross-references autocapture data to name the elements under each hotspot
- **Above/below-the-fold split** – highlights important content sitting below the initial viewport

PostHog AI ranks its recommendations by signal strength: rageclick clusters first, then clicks on non-interactive elements, then CTAs below the scroll cliff.

For more on using PostHog AI with Web Analytics, see [Investigate traffic with PostHog AI](/docs/web-analytics/investigate-traffic-ai).
