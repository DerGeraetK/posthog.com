...
- **Archive** – the flag is disabled and hidden from the flag list, but all configuration and linked data (experiments, surveys) are preserved. Ideal for finished flags where you want to keep the history. You can filter by **Archived** status to view and unarchive them later. See [archiving feature flags](/docs/feature-flags/creating-feature-flags#archiving-feature-flags) for details.
