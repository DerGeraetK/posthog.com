...
Support for this setting with [local evaluation](/docs/feature-flags/local-evaluation) varies by SDK. The [Python](/docs/libraries/python) SDK supports it; other server-side SDKs evaluate all condition sets regardless of this setting, with support rolling out across SDKs. The setting always takes effect when flags are evaluated through the `/flags` endpoint. Check your SDK's documentation before relying on early exit with local evaluation.
