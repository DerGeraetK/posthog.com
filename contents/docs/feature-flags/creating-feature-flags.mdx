...
<CalloutBox icon="IconWarning" title="Limited local evaluation support" type="caution">

Local evaluation supports this setting in the [Python](/docs/libraries/python) SDK only. Other server-side SDKs evaluate all condition sets regardless of this setting – support is rolling out across SDKs. The setting always takes effect when flags are evaluated through the `/flags` endpoint.
