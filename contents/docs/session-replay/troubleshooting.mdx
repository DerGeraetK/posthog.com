...
## Recording can't be played (missing full snapshot)

You may see a message saying "This recording can't be played" because the initial screen snapshot never reached PostHog. This happens when the browser closes or goes offline before the first recording payload finishes uploading.

Session Replay captures an initial DOM snapshot of the page and then records incremental changes. Without this initial snapshot, there's no baseline to apply changes to – nothing can be rendered.

This is most common when:

- The user has a slow internet connection
- The browser closes quickly after page load
- The device goes offline during the initial upload

This data is unrecoverable. If you consistently see this issue, consider optimizing page load performance or ensuring the page has time to complete the initial upload before users navigate away.
