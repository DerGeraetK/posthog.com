...
- **Sentiment analysis** – Classifies the sentiment of user messages on each matching generation as positive, neutral, or negative. Returns a sentiment label and score instead of a pass/fail result, with no prompt or code to write.
...
Sentiment analysis evaluations classify the sentiment of user messages on each matching generation as **positive**, **neutral**, or **negative**. Unlike LLM-as-a-judge and code-based evaluations, they don't return a pass/fail verdict – each run produces a sentiment label and a confidence score. For more on how PostHog classifies sentiment, see [Sentiment classification](/docs/ai-observability/sentiment).

Because sentiment is classified automatically, there's no prompt or code to write. You only choose which generations to evaluate, using the same sampling rate and property filters as other evaluation types.

Sentiment evaluations don't use an LLM judge, so they don't require a provider API key and have no LLM cost. They also don't support N/A results, success rates, or evaluation reports, since those apply only to pass/fail evaluations.
