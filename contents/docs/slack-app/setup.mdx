...
The bot resolves a target with this precedence: thread context (continuing an existing task) → your personal default → workspace-wide default → the only connected project → picker. Personal defaults always win over the workspace fallback, so each teammate can route their own mentions independently.

Slack workspace admins and owners can set a workspace-wide default with `@PostHog project workspace <id>`. This applies to anyone who hasn't set a personal default. You need PostHog access to the target project to set it as the workspace default.
