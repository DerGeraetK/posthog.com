...
PostHog uses parsers to recognize LLM event input and output structures and display them as a readable conversation. When no parser recognizes an event's shape, the input and output are shown as raw JSON with a callout: "Shown as raw JSON — no parser recognizes this event's shape."

To create a custom parser, click **Set up custom parser with PostHog AI** in the callout. [PostHog AI](/docs/posthog-ai) analyzes the event's structure and generates a parser configuration. Once saved, the trace re-renders with the recognized structure displayed as a conversation.
