...
| `execute-sql` | Run a custom HogQL query against your LLM trace data. Supports filtering by model, feature, cost, latency, error status, and time range. |

A typical workflow starts with `get-llm-total-costs-for-project` to check overall spend and identify which models are most expensive, then uses `execute-sql` with a HogQL query to drill into specific traces by feature, time range, or cost threshold.
