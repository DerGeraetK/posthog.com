...
Reply in the thread to send a follow-up while the agent is working. Your message goes straight to the running conversation, so you can steer mid-run ("actually, skip the test files") or answer a question the agent asked.

Once the agent finishes, reply in the thread with `@PostHog` to continue where it left off.

Anyone on your team can jump into the thread. When a teammate sends a follow-up, their name is prefixed to the message (e.g., "Alice: please retry the build") so the agent knows who's talking. The teammate must have access to the same PostHog project.
