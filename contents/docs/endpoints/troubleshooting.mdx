...
- Check the endpoint's [Logs tab](/docs/endpoints/execution-logs) to see `duration_ms` and `cache` values for recent runs
...
## Debugging endpoint failures

When an endpoint starts failing:

1. Open the endpoint and go to the **Logs** tab
2. Filter by **ERROR** level to see failed runs
3. Check the `error` token in the log message to identify the failure type
4. Look at the `version` token to see if a recent version change caused the issue

See [Execution logs](/docs/endpoints/execution-logs) for details on interpreting log entries.
