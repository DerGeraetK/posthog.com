...
LLMs can be tricked into following untrusted commands (prompt injection), so always review tool calls before executing them. You have several ways to limit what an agent can do:

- **Scope reduction at authorization** – When you authorize via OAuth, the consent screen lets you toggle to [read-only mode](/docs/api/oauth#editing-scopes-on-the-consent-screen) (downgrading all write permissions to read) or uncheck individual resources. This is the simplest way to ensure an agent can only read your data.
- **Read-only tools** – Restrict the session to [read-only tools](#restricting-to-read-only-tools) using a header or query parameter, so tools that create, update, or delete data are excluded.
- **Tool filtering** – Restrict the session to specific tools or feature categories using [tool filtering](#filtering-available-tools).
- **Project pinning** – Pin the agent to a specific organization or project to limit blast radius.
