...
## Searching observations with PostHog AI

You can search across scanner observations using natural language with [PostHog AI](/docs/posthog-ai). This enables semantic search over the model's reasoning, letting you find recordings by _what happened_ rather than by specific property values.

### How it works

Every observation's reasoning (or summarizer facets) is embedded for semantic retrieval. When you ask PostHog AI a question like "Find sessions where users appeared confused about pricing," it searches the embeddings to find observations whose reasoning matches your query.

### Where to access it

- **From a scanner page** - A **Chat about these sessions** card appears on monitor, classifier, and scorer pages. Clicking it opens PostHog AI with your search scoped to that scanner.
- **From Session Replay mode** - The `search_replay_vision_observations` tool is available in [Session Replay mode](/docs/posthog-ai/modes#session-replay), so you can search across all your scanners.

### Example prompts

- "What are common trends across sessions that resulted in 'yes'?" (for monitors)
- "What are the common themes across sessions tagged 'frustrated'?" (for classifiers)
- "What's driving the lowest-scoring sessions?" (for scorers)
- "Find sessions where users encountered checkout errors"

### Filtering results

You can combine semantic search with structured filters:

- **Verdict** - Filter monitor results by `yes`, `no`, or `inconclusive`
- **Tags** - Filter classifier results by specific tags
- **Score range** - Filter scorer results by minimum or maximum score

The search is scoped to scanners you have access to view.
