...
You can also use helper methods to set or remove person properties without hand-building `$set`, `$set_once`, or `$unset` payloads.

To set person properties directly, without capturing a separate event, use `setPersonProperties`. This captures a `$set` event with the properties you pass. Properties in the `properties` object behave like `$set`, while those in `propertiesOnce` behave like `$set_once`.
...
To remove person properties, pass the property name (or an array of names) to `unsetPersonProperties`. This captures a `$set` event with a `$unset` array, which removes those properties from the person's profile.
...
client.unsetPersonProperties({
  distinctId: 'distinct_id_of_the_user',
  properties: ['name', 'initial_url'],
})
```

For more examples, see our [person properties docs](/docs/product-analytics/person-properties).
