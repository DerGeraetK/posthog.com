...
You can also set person properties directly, without capturing a separate event, using `setPersonProperties`. This captures a `$set` event with the properties you pass. Properties in the `properties` object behave like `$set`, while those in `propertiesOnce` behave like `$set_once`.
