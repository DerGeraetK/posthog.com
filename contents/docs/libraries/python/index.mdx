...
The `before_send` callback lets you modify or filter events before they're sent to PostHog. Common use cases include:

- **Privacy** - Removing or masking sensitive data (PII)
- **Filtering** - Dropping unwanted events (test events, internal users, bot traffic)
- **Enhancement** - Adding custom properties to all events
- **Transformation** - Modifying event names or property formats

Pass a `before_send` function when initializing the client. It receives the event dictionary and returns either the modified event or `None` to drop it:
...
def my_before_send(event: dict[str, Any]) -> Optional[dict[str, Any]]:
    ...
    if properties.get("is_internal_user"):
        return None
    
    # Mask email addresses but keep the domain
...
**Filter events by property or name:**

```python
def filter_events(event: dict[str, Any]) -> Optional[dict[str, Any]]:
    properties = event.get("properties", {})
    
    # Drop events from internal sources
    if properties.get("source", "").endswith("internal"):
        return None
...
def filter_traffic(event: dict[str, Any]) -> Optional[dict[str, Any]]:
    properties = event.get("properties", {})
...
```python
def log_and_drop_in_dev(event: dict[str, Any]) -> Optional[dict[str, Any]]:
    print(event)
    return None  # Drop all events in dev mode
```

### Error handling

If your `before_send` function raises an exception, the SDK logs the error and sends the original event unchanged. Your application won't crash:

```python
def risky_before_send(event: dict[str, Any]) -> Optional[dict[str, Any]]:
    # If this raises, the original event is sent anyway
    risky_operation()
    return event
```
