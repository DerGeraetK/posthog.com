...
The Python SDK has opt-in burst protection that limits autocaptured exceptions per exception type. This prevents an infinite loop or crash from flooding your project with duplicate exceptions.
...
Each exception type gets its own token bucket. By default, each bucket allows 49 exceptions before rate limiting kicks in, then refills 10 tokens every 10 seconds. Exceptions that exceed the limit are dropped before reaching the ingestion queue.
...
posthog = Posthog(
    ...
    enable_exception_autocapture=True,
    enable_exception_autocapture_rate_limiting=True,
    exception_autocapture_bucket_size=50,
    exception_autocapture_refill_rate=10,
...
|---|---|---|
| `enable_exception_autocapture_rate_limiting` | `False` | Enable client-side rate limiting for autocaptured exceptions. |
| `exception_autocapture_bucket_size` | `50` | Maximum burst of exceptions allowed per exception type (clamped to 0–100). |
| `exception_autocapture_refill_rate` | `10` | Tokens restored per refill interval. |
| `exception_autocapture_refill_interval_seconds` | `10` | Seconds between token refills. |

Burst protection runs client-side, per process. To cap exception volume across your whole project, set a server-side [rate limit](/docs/error-tracking/rate-limiting).
