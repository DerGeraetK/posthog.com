...
When you switch a field to use Liquid templating, use double braces `{{ }}` instead of single braces:

| Engine | Syntax | Example |
|--------|--------|---------|
| Hog (default) | Single braces `{ }` | `{person.properties.email}` |
| Liquid | Double braces `{{ }}` | `{{ person.properties.email }}` |

Expressions without any braces (like `person.properties.email`) are sent as literal text in both engines.
