...
3. Enter your Supabase connection details. You can find these in your Supabase project under **Project Settings** → **Database** → **Connection pooling**:
   - **Host:** Use the **Session pooler** host, like `aws-0-us-east-1.pooler.supabase.com`. Don't use the direct database host (`db.<ref>.supabase.co`) — it's IPv6-only and won't connect from PostHog.
   ...
   - **User:** The pooler username in the format `postgres.<project-ref>` (not just `postgres`).
...
> **Note:** PostHog connects over IPv4. The direct Supabase database host (`db.<ref>.supabase.co`) is IPv6-only and won't work. Use the Session pooler host as described above, which supports IPv4 connections.
