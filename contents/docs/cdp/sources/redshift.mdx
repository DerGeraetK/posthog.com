...
3. Enter your database connection details:
    ...
    - **Schema:** (Optional) The schema where your tables are located. Leave blank to import tables from all schemas, or specify a single schema like `public` to import only from that schema.
4. Click **Next**
...
Leave the **Schema** field blank to import tables from all non-system schemas. Tables appear with qualified names like `schema.table` (for example, `analytics.users` or `reporting.orders`), so tables with the same name in different schemas won't conflict.

PostHog automatically excludes system schemas like `pg_catalog`, `information_schema`, and `pg_internal`.
