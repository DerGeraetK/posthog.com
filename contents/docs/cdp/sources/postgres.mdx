...
3. Select a column, choose an operator (`=`, `!=`, `>`, `>=`, `<`, `<=`, `IN`, `NOT IN`), and enter a value
...
Row filters support integer, numeric, string, boolean, date (`YYYY-MM-DD`), and timestamp (`YYYY-MM-DDTHH:MM:SS`) columns. Filters are applied on the next sync and don't remove rows that were already synced. They aren't available for tables synced with [change data capture (CDC)](#how-posthog-cdc-works).
