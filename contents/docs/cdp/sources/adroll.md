...
NextRoll uses a dual auth scheme: the personal access token is sent as the `Authorization` header, and the Client ID is sent as the `apikey` query parameter on every request.
