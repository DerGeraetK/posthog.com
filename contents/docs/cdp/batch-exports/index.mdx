...
### Why can't I set an end date in the future?

Batch exports only export data that already exists in PostHog. When you specify a `data_interval_end` – whether through backfills or the on-demand export API – it must be a time in the past. If you try to set a future end date, you'll get an error.

This keeps batch exports from waiting around indefinitely for events that haven't happened yet.
