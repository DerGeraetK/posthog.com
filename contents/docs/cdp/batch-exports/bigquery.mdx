...
2. A dataset that grants the service account access.

PostHog uses **service account impersonation** to connect to your BigQuery account. You provide a service account email and project ID, and PostHog impersonates your service account using its own credentials. No service account key is required.

### Service account permissions

You need a service account with permissions to load data in BigQuery by running BigQuery jobs. Follow these steps:
...
### Granting PostHog access to impersonate your service account

Grant PostHog permission to impersonate your service account by following these steps:
...
3. When creating the batch export, connect to a new Google Cloud service account and provide your service account email and Google Cloud project ID.
...
To connect your Google Cloud service account, provide the **Service Account Email** and **Project ID**. See the [BigQuery access setup section](#setting-up-bigquery-access) for more information on configuring authentication.
