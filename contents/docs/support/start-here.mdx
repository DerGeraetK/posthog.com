...
Two ways messages become tickets:

1. **Support channel** – every message in a dedicated channel becomes a ticket
2. **Bot mention** – `@SupportHog` in any channel

Shared channels use polling instead of real-time webhooks due to Microsoft API limitations. PostHog polls them via Microsoft Graph every minute to pick up new messages. Shared channels show a **Shared · polled** badge in the settings UI.
