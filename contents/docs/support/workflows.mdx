...
| `actor_id` | int or null | PostHog user ID if `actor_type` is `user`, otherwise null |
| `actor_email` | string or null | Email of the actor if `actor_type` is `user`, otherwise null |
