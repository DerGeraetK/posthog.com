...
## Customizing the person attribute key

By default, PostHog links logs to person profiles using the `posthogDistinctId` attribute – the key the JavaScript and React Native SDKs attach automatically. If your pipeline emits the person identifier under a different attribute key, you can change this in [**Settings** > **Environment** > **Logs** > **Person pivot attribute**](https://app.posthog.com/settings/environment-logs#logs-distinct-id-attribute-key).

> **Note:** Changing this setting requires project admin permissions.
