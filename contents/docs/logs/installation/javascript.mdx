...
| `beforeSend` | Pre-send filter to inspect, redact, or drop log records. See [Filter and redact logs with `beforeSend`](#filter-and-redact-logs-with-beforesend) below. |
...
<Step title="Filter and redact logs with beforeSend" badge="optional">

Use `logs.beforeSend` to inspect, redact, or drop log records before they're sent. It runs for records from both `posthog.logger` and `posthog.captureLog`, so it's the right place to strip sensitive data or filter out noise, no matter how a record was created.
...
posthog.init('<ph_project_token>', {
  ...
  logs: {
    serviceName: 'checkout-web',
    beforeSend: (log) => {
      // Drop records that contain sensitive data
      if (log.body.includes('password')) return null

      // Or return a modified copy to redact it
      return {
        ...log,
        body: log.body.replace(/token=\S+/g, 'token=[REDACTED]'),
      }
    },
  },
})
...
To keep a record, return it (modified or not). To drop it, return `null`. You can also pass an array of functions to form a chain that's evaluated left to right – returning `null` from any function short-circuits the chain and drops the record. If a function throws, the record is dropped rather than crashing your app.
