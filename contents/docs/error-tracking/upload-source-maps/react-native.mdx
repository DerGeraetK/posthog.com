...
| `skipOnConflict` | `boolean` | `false` | When `true`, passes `--skip-on-conflict` to the source map upload (`posthog-cli hermes upload`) on both iOS and Android, so duplicate uploads are skipped instead of failing the build. Useful when the same source maps may be uploaded more than once. |
