...
| `skipOnConflict` | `boolean` | `false` | When `true`, passes `--skip-on-conflict` to the source map upload (`posthog-cli hermes upload`) on both iOS and Android, so duplicate uploads are skipped instead of failing the build. Use this when the same build can upload the same source maps more than once. Requires `posthog-react-native` 4.49.1 or later. |
