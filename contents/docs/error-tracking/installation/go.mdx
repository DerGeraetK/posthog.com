...
<CalloutBox icon="IconInfo" title="Build with a GNU build ID for full symbolication" type="fyi">

Go binaries don't include a GNU build ID by default. Without one, PostHog can't match uploaded debug symbols to crash events. To enable native symbolication, compile with `-ldflags "-B gobuildid"`:

```bash
go build -ldflags "-B gobuildid" -o myapp .
```

Then [upload your debug symbols](/docs/error-tracking/stack-traces#uploading-source-maps) using the `posthog-cli symbol-sets upload` command.
