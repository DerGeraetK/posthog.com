...
Dot notation only works when property keys are valid identifiers – they must start with a letter, underscore, or `$` and contain only letters, digits, underscores, and `$`.

Use bracket notation for keys that contain hyphens, dots, spaces, or start with a digit. For example, survey response properties like `$survey_response_0196e96d-7705-7d32` contain hyphens, so dot notation fails:

```sql
-- ❌ Fails: hyphens are parsed as subtraction
SELECT properties.$survey_response_0196e96d-7705-7d32 FROM events

-- ✅ Works: bracket notation with quoted string
SELECT properties['$survey_response_0196e96d-7705-7d32'] FROM events
```

This applies to all JSON columns, including `person.properties` and data warehouse sources.
