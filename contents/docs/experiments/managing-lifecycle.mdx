...
4. Archiving the experiment. When you archive an experiment, PostHog also archives its linked feature flag automatically — but only if the flag is already disabled and no other live experiment uses it. If the flag is still enabled (e.g., you're rolling out the winning variant), it's left untouched. Unarchiving the experiment unarchives the flag too (it stays disabled).
