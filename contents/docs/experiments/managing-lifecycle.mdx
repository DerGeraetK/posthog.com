...
4. Archiving the experiment. When you archive an experiment, PostHog also archives its linked feature flag – but only if the flag is disabled and no other running experiment uses it. This keeps your flag list clean without extra work. If you unarchive the experiment later, the flag is unarchived too (it stays disabled until you manually re-enable it).
