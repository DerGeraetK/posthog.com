...
### Permission denied when creating a metric

**Problem:** You receive a "You don't have access to data warehouse table(s)" error when creating or updating an experiment metric that uses a Data Warehouse table.

**Solution:** You need read access to the Data Warehouse table referenced in your metric. Check with your organization admin to ensure you have the appropriate permissions. The table picker in the PostHog UI only displays tables you have access to, but if you're using the API directly, you must have permission to access any referenced tables.
