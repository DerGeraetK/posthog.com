...
When viewing experiment results in a [dashboard widget](/docs/product-analytics/dashboards#adding-widgets), both primary and secondary metrics are displayed. Each section shows up to three metrics. If an experiment has more than three metrics in either section, the widget displays "Showing the first N of M metrics. Open the experiment to see all of them." The full experiment page shows all metrics without this limit.
