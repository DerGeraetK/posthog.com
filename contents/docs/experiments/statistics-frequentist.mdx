...
The effect size quantifies how much better (or worse) the treatment variant performs compared to the baseline. PostHog uses relative differences by default, which express changes as percentages:

```
effect_size = (treatment_mean - baseline_mean) / baseline_mean
```

For example, if the baseline has a 10% conversion rate and the treatment has 12%, the relative difference is +20%. This is what PostHog displays in the UI (e.g., "Variant B: +20% conversion rate"). Relative differences are intuitive for business metrics because they show the proportional change, making it easy to understand the business impact.
...
- Each variant is compared to the baseline independently
- The p-value and confidence interval are calculated for each variant vs. the baseline
