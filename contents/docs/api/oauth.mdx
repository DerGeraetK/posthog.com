...
PostHog also serves an [OAuth 2.0 Protected Resource Metadata (RFC 9728)](https://datatracker.ietf.org/doc/html/rfc9728) document. It tells clients which authorization server issues tokens for the API, which scopes are available, and how to present the token. If a client receives a `401 Unauthorized` response, it can follow the `WWW-Authenticate` header to this document to find where to authenticate.
...
- `resource` – The identifier of this PostHog instance
- `authorization_servers` – The authorization server that issues tokens for this resource
- `scopes_supported` – Available OAuth scopes (see below)
- `bearer_methods_supported` – How to present the access token
...
- [OAuth 2.0 Protected Resource Metadata (RFC 9728)](https://datatracker.ietf.org/doc/html/rfc9728) – Resource metadata discovery via `.well-known`
