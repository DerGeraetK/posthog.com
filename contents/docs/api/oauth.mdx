...
## Editing scopes on the consent screen

When authorizing an application, you can customize the permissions you grant. The consent screen gives you two ways to reduce requested scopes:

- **Read-only mode** – If the application requests write scopes, a toggle lets you downgrade all write access to read access. For example, `feature_flag:write` becomes `feature_flag:read`.
- **Per-resource opt-out** – Each requested resource appears as a checkbox you can uncheck to remove that permission entirely.

Identity scopes (`openid`, `profile`, `email`) are always granted and can't be removed.

These controls only let you narrow what an application receives – never widen it. The application still operates under the server-side scope ceiling, so the final grant is the intersection of what you approve and what the application is allowed to request.
