...
Both are POST-only public endpoints that use your [project token](https://app.posthog.com/project/settings) and do not return any sensitive data from your PostHog instance. You can also use your project's **secret API token** (`phs_`) to capture [verified events](/docs/data/events#verified-events) from trusted server infrastructure.
