...
PostHog sends email notifications when data warehouse syncs fail. The email lists failing syncs grouped by source and shows whether each sync is paused (requiring action) or will retry automatically.

You receive at most one notification per day. To manage these notifications, go to [**Account > Notifications**](https://app.posthog.com/settings/user#notifications) and toggle **Data pipeline errors**.
