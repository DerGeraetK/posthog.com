...
| Tool                                     | Description                                                                                                                               |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `data-warehouse-source-setup`            | Create a source in one step: validates credentials, discovers tables, applies sync defaults, and creates the source.                      |
| `data-warehouse-source-connect-link`     | Get a secure browser link for the user to authorize OAuth or enter credentials – secrets never pass through the agent.                    |
| `data-warehouse-stored-credentials-list` | List credentials the user stored via the connect link. Pass the `credential_id` to `data-warehouse-source-setup` to complete the import.  |
...
| `external-data-sources-create`           | Create a new source connection (advanced). Use `data-warehouse-source-setup` for most cases.                                              |
| `external-data-sources-partial-update`   | Update a source's prefix or description.                                                                                                  |
...
- `I want to connect my Stripe account to query payment data.`
- `Set up a Postgres source so I can query my production database.`
- `Help me import my Hubspot contacts into the data warehouse.`
