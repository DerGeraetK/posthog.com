...
The **Completed an event for first time** filter supports custom date ranges, including relative dates (like "last 30 days") and specific date ranges (like "Jan 1 to Jan 31, 2024"). The other lifecycle filters accept `days`, `weeks`, `months` or `years` time ranges.
