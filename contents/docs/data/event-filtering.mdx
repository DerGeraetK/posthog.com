...
1. **Field** – what to match on:
   ...
   - `ip` – the client IP address the event was captured from, useful for dropping traffic from a known-bad source address
...
- Only `event_name`, `distinct_id`, and `ip` fields are supported
