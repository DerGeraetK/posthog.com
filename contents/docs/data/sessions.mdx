...
If you enable tracing headers within the JavaScript SDK, the session ID will be sent on every request. You can enable tracing headers by adding the `tracing_headers` option to your `posthog.init` call:
