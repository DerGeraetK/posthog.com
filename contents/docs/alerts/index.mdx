...
10. Pick who should be notified when the alert is triggered. Subscribed users automatically receive in-app notifications when the alert fires. You can also add email recipients, Slack channels, Discord channels, or webhook URLs directly in the alert form.
...
3. For Discord notifications:
   - Enter the [incoming webhook URL](https://support.discord.com/hc/en-us/articles/228383668-Intro-to-Webhooks) for the channel where you want alerts sent. Discord only needs a webhook URL, so there's no integration to connect.
