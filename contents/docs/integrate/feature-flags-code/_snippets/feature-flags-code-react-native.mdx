...
```react-native
const result = posthog.getFeatureFlagResult('key-for-your-multivariate-flag')

if (result?.variant === 'variant-name') { // replace 'variant-name' with the name of your variant
  const payload = result.payload
  // Do something with the variant and payload
}
```

The result includes the flag's `key`, `enabled`, `variant`, and `payload`. `getFeatureFlagPayload` still works, but it's deprecated in favor of `getFeatureFlagResult`.
