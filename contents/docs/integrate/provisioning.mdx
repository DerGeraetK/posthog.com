...
| `link_method`                     | string | No       | How to handle existing accounts: `"browser"` (default) redirects to a consent page; `"email_code"` emails the user a one-time code they can enter in the terminal. See [email code linking](#email-code-linking). |
...
**Existing user response with email code** (HTTP 200):

If you set `link_method` to `"email_code"` and your partner app has email code linking enabled, you'll get this response instead:
...
When `type` is `requires_otp`, PostHog has emailed a one-time code to the account owner. The email includes the partner name and the exact scopes being granted. Prompt the user to enter the code, then use it as the authorization code in [step 2](#step-2-exchange-the-code-for-tokens). See [email code linking](#email-code-linking) for the full flow.
...
CLI and agent-driven integrations can't redirect users to a browser without breaking the terminal flow. Email code linking lets existing users authorize entirely in the terminal by entering a one-time code from their inbox.

### How it works

1. Your app calls `/account_requests` with `link_method: "email_code"` and a PKCE `code_challenge`.
2. PostHog emails the account owner a one-time code along with your app's name and the exact scopes being granted.
3. The API returns `{"type": "requires_otp", "requires_otp": {"expires_in": 600}}`.
4. Your app prompts the user (or their agent) to enter the code.
5. Your app exchanges the code at `/oauth/token` with the PKCE `code_verifier`, just like the browser flow.

```mermaid
sequenceDiagram
    participant P as Partner CLI
    participant PH as PostHog
    participant U as User Inbox

    P->>PH: POST /account_requests (link_method=email_code, code_challenge)
    PH->>U: Email with one-time code + scope disclosure
    PH-->>P: requires_otp (expires_in: 600)

    U-->>P: User enters code
    P->>PH: POST /oauth/token (code, code_verifier)
    PH-->>P: access_token + refresh_token
```
...
The emailed code doubles as the authorization code. Redemption requires both the code (proving email control) and the PKCE verifier (proving it's the same session that started the request). An intercepted email alone isn't enough to mint a token.

The email also serves as the consent surface: it names your app and lists the exact scopes being granted, so the user knows what they're authorizing before entering the code.

### Requirements

- Your partner app must have `provisioning_can_link_via_email_code` enabled. Contact PostHog to request access.
- You must include a non-empty `scopes` array in the request. Empty scopes are rejected because the email must disclose exactly what's being granted.
- Rate limit: 3 codes per email address per hour. After that, requests return HTTP 429 with a `Retry-After` header.

### Fallback behavior

If your app doesn't have email code linking enabled, or if you omit `link_method` or set it to `"browser"`, the API falls back to `requires_auth` with a browser consent URL. Your client should handle both response types.
...
**Email code linking** (per email address):

- 3 codes per hour per recipient address
