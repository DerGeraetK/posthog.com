...
| `label_prefix` | string | No | Label prefix for the provisioned personal API key, up to 25 characters. Only applies when PAT minting is enabled for the app (off by default). The key is labeled `{label_prefix} - {team_name}`; if omitted, empty, or whitespace-only, just the team name. |
...
> **Note:** The `personal_api_key` field is only included when PAT minting is enabled for the partner app. Most new apps won't receive this field – use the OAuth access token for API calls instead.
...
| `personal_api_key` | (Optional) A personal API key scoped to this project. Only returned for apps with PAT minting enabled (off by default). |

For authenticated REST API calls, use the OAuth `access_token` from [Step 2](#step-2-exchange-the-code-for-tokens) as a `Bearer` token. It carries the [scopes](#available-scopes) you requested in the account request.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `label_prefix` | string | No | Label prefix for the new personal API key, up to 25 characters. Only applies when PAT minting is enabled for the app (off by default). The key is labeled `{label_prefix} - {team_name}`; if omitted, empty, or whitespace-only, just the team name. |

The response has the same shape as the project provisioning response and includes the rotated `api_key` and `host`. If PAT minting is enabled for the app, the response also includes a new `personal_api_key`.
...
async function provisionPostHogAccount(email, name) {
  ...
  return {
    ...
    // personal_api_key is only present if PAT minting is enabled for the app
