...
| `label_prefix` | string | No | Label prefix for the personal API key shown in PostHog, up to 25 characters. Only applies if the app has PAT minting enabled. The key is labeled `{label_prefix} - {team_name}`. If omitted, empty, or whitespace-only, the key is labeled with just the team name. |
...
| `personal_api_key` | (Optional) A personal API key scoped to this project. Only returned for apps with PAT minting enabled. Most apps should use the OAuth access token for API calls instead. |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `label_prefix` | string | No | Label prefix for the new personal API key shown in PostHog, up to 25 characters. Only applies if the app has PAT minting enabled. The key is labeled `{label_prefix} - {team_name}`. If omitted, empty, or whitespace-only, the key is labeled with just the team name. |
