...
2. Add a `com.posthog` object to your CIMD metadata document with a `verification_token` field:
...
3. The next time PostHog refreshes the metadata document, the app is linked to the matching organization and the rate limit increases.

The token only proves ownership of the partner app – it isn't sent on API requests.
...
> **Note:** The legacy top-level `posthog_verification_token` field is still supported for backwards compatibility, but `com.posthog.verification_token` is preferred. If both are present, the nested form takes precedence.

### Set a scope ceiling for your app (optional)

Limit which OAuth scopes your partner app can request by declaring them in your CIMD metadata. This sets a ceiling – users can still grant fewer scopes, but your app can't request more than what's declared.

Add a `scopes` array inside the `com.posthog` object:
...
```json
{
  ...
  "com.posthog": {
    "verification_token": "phvt_...",
    "scopes": ["insight:read", "person:read", "project:read"]
  }
}
```

PostHog only allows unprivileged scopes in this list. Privileged, internal, or unrecognized scope strings are silently filtered out. See [available scopes](#available-scopes) for the full list.

When scopes are declared:
- On first registration, the app is created with these scopes as its ceiling
- On metadata refresh, the scopes are updated to match the new declaration

When scopes are absent from the metadata:
- On first registration, the app gets no scope ceiling (the default set is granted at authorization time)
- On metadata refresh, existing scopes are left unchanged
