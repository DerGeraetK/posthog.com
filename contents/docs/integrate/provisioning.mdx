...
Authenticate with the `access_token` you got from [Step 2](#step-2-exchange-the-code-for-tokens) – the same Bearer credential you use for `/resources`. The token is scoped to a single team. Use the optional `path` parameter to send users directly to a specific page like a session recording, insight, or settings screen.
