...
Click the PostHog app in your Slack sidebar to open the App Home tab. The **Tasks** card lists recent tasks you started by mentioning `@PostHog` in the workspace, scoped to teams you can access.

Each task row shows a linked title (deep-linked to PostHog), the repo name, current status, and links to the Slack thread and associated PR.

Use the controls at the top of the card to narrow the list:

- **Repo dropdown** – filter by repository. Shows an "All repos" option to reset.
- **Status dropdown** – filter by status (in progress, done, failed, cancelled, queued, or not started).
- **Refresh** – reload the list without leaving the Home tab.

Tasks that haven't had any Slack thread activity in the last 14 days automatically drop off the list.
