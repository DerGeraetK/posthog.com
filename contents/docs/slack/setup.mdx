...
You need two integrations to use `@PostHog` in Slack: one workspace-wide Slack connection per PostHog project (set up by a project admin), and either a personal GitHub connection per teammate who wants PRs under their own name, or a team-level GitHub integration so teammates can author PRs as the PostHog bot.
...
<Step title="Connect your personal GitHub" badge="recommended">

Connect your personal GitHub so the agent commits and opens PRs as you. Each teammate who wants PRs under their own name connects their account at [**Settings → Personal integrations**](https://app.posthog.com/settings/user-personal-integrations) and clicks **Connect GitHub**. You can connect multiple accounts or organizations – the agent uses them for repo access, commit attribution, and PR authorship.

### Without a personal GitHub connection

If your team has an organization-level GitHub integration but you haven't connected a personal account, the bot prompts you to choose:

> You have no personal integration setup yet. The PR will be authored by the PostHog bot. To change this, set up a personal integration.

Click **Continue as PostHog** to let the agent author the PR under the PostHog bot. Or click **Connect GitHub** to link your personal account first – the task waits for you to finish the OAuth flow. This prompt times out after 15 minutes; re-mention the bot to try again.

If there's no team GitHub integration either, personal GitHub is required – the bot won't start until you connect.
...
If you start a task before connecting and no team GitHub integration exists, the agent stops and posts a **Connect GitHub** button in the thread. Click it, finish the OAuth flow, and re-mention the bot – the task picks up where it left off.
