...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| [Triggers](#triggers)                 | What starts the workflow. You can trigger a workflow from an event, a batch audience on a recurring schedule, a webhook, or a tracking pixel. |
...
| Batch trigger          | Run the workflow for each person matching an audience, once or on a recurring schedule            |
...
Batch triggers let you run a workflow for each person in an audience you define, either once or on a recurring schedule.
Each time the batch runs, PostHog re-evaluates your audience filters and executes the workflow once per matching person. This means a recurring batch picks up new people who have entered the audience since the last run.
...
- **A schedule** — when the batch should run

#### One-off vs. recurring batches

By default, the **Schedule** section lets you pick a single date and time, which runs the batch once.

To run the batch on a repeating interval, turn on the **Repeat** toggle next to the schedule. This reveals the recurring schedule controls, where you can set the interval (e.g. every day, every week on specific days, every month), a timezone, and an optional end condition (never, on a specific date, or after a number of occurrences). A summary of the next occurrences is shown so you can confirm the schedule before enabling the workflow.

If you want a batch that regularly checks who is in an audience and adds new matching people to the workflow, set a start date and time, then enable **Repeat** and choose your interval.
