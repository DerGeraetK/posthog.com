...
Feature flags can't be used as a batch audience filter because flag evaluation is dynamic (rollout percentages, holdouts, and `distinct_id` hashing), so it can't produce a static, queryable audience.
