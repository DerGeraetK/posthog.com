...
When you create a new scanner, a three-step wizard guides you through the process:

1. **Choose a template** – pick a pre-configured template (dead-end detection, session summary, user intent, frustration score, or session outcome) or select **Create from scratch** to start with a blank scanner. Templates pre-fill the scanner type, prompt, and type-specific options. Starting from scratch lets you choose the [scanner type](#scanner-types) yourself.
2. **Configure** – give the scanner a name, review or write its prompt, and set type-specific options:
...
3. **Triggers** – set **recording filters** to choose which sessions the scanner applies to (by person properties, session properties, cohorts, or events) and a **sampling rate** to control how many matching sessions get scanned. Optionally enable [Signals](#signals) so notable findings can trigger alerts.

Save to finish. The scanner is enabled by default.
