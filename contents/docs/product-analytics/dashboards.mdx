...
Widget tiles embed live data from other PostHog features directly on a dashboard without creating a separate insight. They're useful for getting a quick view of recent activity.

Click the dropdown on the **Add...** button and select the widget you want to add:

- **Recent events** – Shows the latest events captured in your project, similar to the [Activity > Explore](/docs/data/events) view. Displays event name, person, URL/screen, and timestamp for each event. Use the date range filter on the tile to adjust the time window (default: last 24 hours).
- **Error tracking issues** – Shows recent issues from [Error tracking](/docs/error-tracking). Requires Error tracking to be set up.
- **Session recordings** – Shows recent recordings from [Session replay](/docs/session-replay). Requires Session replay to be enabled.
- **Experiments list** – Shows your [experiments](/docs/experiments) filtered by status (draft, running, paused, stopped, or all) and optionally by creator. Each row links to the experiment detail page.
- **Experiment results** – Shows the primary metrics and current results for a selected experiment, up to three metrics with their statistical summaries. Configure the tile to choose which experiment to display.

Widget tiles update when you refresh the dashboard. Click the tile title to jump to the full view for that feature (e.g., Activity > Explore for recent events, or the experiment detail page for experiment widgets).
