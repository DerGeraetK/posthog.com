...
Every Slack subscription message includes an in-thread hint that nudges anyone in the channel to dig into the report. Reply in the message's thread and mention [`@PostHog`](/docs/slack) with a follow-up question – for example, "Why did signups drop last Tuesday?" – and the [@PostHog bot](/docs/slack) answers it right there in the thread. The hint appears at the bottom of each subscription message when the [PostHog Slack app](/docs/slack) is fully configured with the required scopes.

This works for both insight and dashboard subscriptions and [prompt subscriptions](#prompt-subscriptions).
