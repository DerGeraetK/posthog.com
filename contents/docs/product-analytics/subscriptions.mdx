...
Every Slack subscription message includes an in-thread hint that nudges anyone in the channel to dig into the report. Reply in the message's thread and mention **@PostHog** with a question – for example, "Why did signups drop last Tuesday?" – and the [@PostHog bot](/docs/slack) answers it right there in the thread.

This works for both insight and dashboard subscriptions and [prompt subscriptions](#prompt-subscriptions).

If the PostHog bot isn't fully set up in your workspace, the hint links to the [setup docs](/docs/slack/setup) instead.
